...
import GEOIP.const
from GEOIP.util import ip2long
from GEOIP.timezone import time_zone_by_country_and_region

import GEOIP.six

MMAP_CACHE = GEOIP.const.MMAP_CACHE
MEMORY_CACHE = GEOIP.const.MEMORY_CACHE
STANDARD = GEOIP.const.STANDARD
...
class GeoIP(GeoIPBase):
    ...
    def __init__(self, filename, flags=0):
        ...
        if self._flags & GEOIP.const.MMAP_CACHE:
            ...
            if self._flags & GEOIP.const.MEMORY_CACHE:
    ...
    def _setup_segments(self):
        ...
        """
        self._databaseType = GEOIP.const.COUNTRY_EDITION
        self._recordLength = GEOIP.const.STANDARD_RECORD_LENGTH

        filepos = self._filehandle.tell()
        self._filehandle.seek(-3, os.SEEK_END)

        for i in range(GEOIP.const.STRUCTURE_INFO_MAX_SIZE):
            delim = self._filehandle.read(3)

            if delim == GEOIP.six.b(chr(255) * 3):
                ...
                if self._databaseType == GEOIP.const.REGION_EDITION_REV0:
                    self._databaseSegments = GEOIP.const.STATE_BEGIN_REV0

                elif self._databaseType == GEOIP.const.REGION_EDITION_REV1:
                    self._databaseSegments = GEOIP.const.STATE_BEGIN_REV1

                elif self._databaseType in (GEOIP.const.CITY_EDITION_REV0,
                                            GEOIP.const.CITY_EDITION_REV1,
                                            GEOIP.const.ORG_EDITION,
                                            GEOIP.const.ISP_EDITION,
                                            GEOIP.const.ASNUM_EDITION):
                    self._databaseSegments = 0
                    buf = self._filehandle.read(GEOIP.const.SEGMENT_RECORD_LENGTH)

                    for j in range(GEOIP.const.SEGMENT_RECORD_LENGTH):
                        if GEOIP.six.PY3:
                            self._databaseSegments += (buf[j] << (j * 8))
                        else:
                            self._databaseSegments += (ord(buf[j]) << (j * 8))

                    if self._databaseType in (GEOIP.const.ORG_EDITION, GEOIP.const.ISP_EDITION):
                        self._recordLength = GEOIP.const.ORG_RECORD_LENGTH
        ...
        if self._databaseType == GEOIP.const.COUNTRY_EDITION:
            self._databaseSegments = GEOIP.const.COUNTRY_BEGIN
    ...
    def _get_org(self, ipnum):
        ...
        org_buf = self._filehandle.read(GEOIP.const.MAX_ORG_RECORD_LENGTH)
        if GEOIP.six.PY3:
    ...
    def _get_region(self, ipnum):
        ...
        if self._databaseType == GEOIP.const.REGION_EDITION_REV0:
            ...
            seek_region = seek_country - GEOIP.const.STATE_BEGIN_REV0
            if seek_region >= 1000:
                country_code = 'US'
                region = ''.join([chr((seek_region // 1000) // 26 + 65), chr((seek_region // 1000) % 26 + 65)])
            else:
                country_code = GEOIP.const.COUNTRY_CODES[seek_region]
                region = ''
        elif self._databaseType == GEOIP.const.REGION_EDITION_REV1:
            seek_country = self._seek_country(ipnum)
            seek_region = seek_country - GEOIP.const.STATE_BEGIN_REV1
            if seek_region < GEOIP.const.US_OFFSET:
                country_code = '';
                region = ''
            elif seek_region < GEOIP.const.CANADA_OFFSET:
                country_code = 'US'
                region = ''.join([chr((seek_region - GEOIP.const.US_OFFSET) // 26 + 65), chr((seek_region - GEOIP.const.US_OFFSET) % 26 + 65)])
            elif seek_region  < GEOIP.const.WORLD_OFFSET:
                country_code = 'CA'
                region = ''.join([chr((seek_region - GEOIP.const.CANADA_OFFSET) // 26 + 65), chr((seek_region - GEOIP.const.CANADA_OFFSET) % 26 + 65)])
            else:
                i = (seek_region - GEOIP.const.WORLD_OFFSET) // GEOIP.const.FIPS_RANGE
                if i in GEOIP.const.COUNTRY_CODES:
                    country_code = GEOIP.const.COUNTRY_CODES[(seek_region - GEOIP.const.WORLD_OFFSET) // GEOIP.const.FIPS_RANGE]
                else:
                    country_code = ''
                region = ''

        elif self._databaseType in (GEOIP.const.CITY_EDITION_REV0, GEOIP.const.CITY_EDITION_REV1):
    ...
    def _get_record(self, ipnum):
        ...
        self._filehandle.seek(record_pointer, os.SEEK_SET)
        record_buf = self._filehandle.read(GEOIP.const.FULL_RECORD_LENGTH)

        record = {}

        record_buf_pos = 0
        char = record_buf[record_buf_pos] if GEOIP.six.PY3 else ord(record_buf[record_buf_pos])
        record['country_code'] = GEOIP.const.COUNTRY_CODES[char]
        record['country_code3'] = GEOIP.const.COUNTRY_CODES3[char]
        record['country_name'] = GEOIP.const.COUNTRY_NAMES[char]
        record_buf_pos += 1
        str_length = 0

        # get region
        char = record_buf[record_buf_pos+str_length] if GEOIP.six.PY3 else ord(record_buf[record_buf_pos+str_length])
        while (char != 0):
            str_length += 1
            char = record_buf[record_buf_pos+str_length] if GEOIP.six.PY3 else ord(record_buf[record_buf_pos+str_length])
        ...
        if self._databaseType == GEOIP.const.CITY_EDITION_REV1:
            dmaarea_combo = 0
        ...
        if 'dma_code' in record and record['dma_code'] in GEOIP.const.DMA_MAP:
            record['metro_code'] = GEOIP.const.DMA_MAP[record['dma_code']]
        else:
            record['metro_code'] = ''
    ...
    def country_code_by_addr(self, addr):
        ...
        try:
            if self._databaseType == GEOIP.const.COUNTRY_EDITION:
                ...
                return GEOIP.const.COUNTRY_CODES[country_id]
            elif self._databaseType in (GEOIP.const.REGION_EDITION_REV0, GEOIP.const.REGION_EDITION_REV1,
                                          GEOIP.const.CITY_EDITION_REV0, GEOIP.const.CITY_EDITION_REV1):
    ...
    def country_name_by_addr(self, addr):
        ...
        try:
            if self._databaseType == GEOIP.const.COUNTRY_EDITION:
                country_id = self._lookup_country_id(addr)
                return GEOIP.const.COUNTRY_NAMES[country_id]
            elif self._databaseType in (GEOIP.const.CITY_EDITION_REV0, GEOIP.const.CITY_EDITION_REV1):
    ...
    def region_by_addr(self, addr):
        ...
        try:
            ...
            if not self._databaseType in (GEOIP.const.REGION_EDITION_REV0, GEOIP.const.REGION_EDITION_REV1,
                                          GEOIP.const.CITY_EDITION_REV0, GEOIP.const.CITY_EDITION_REV1):
